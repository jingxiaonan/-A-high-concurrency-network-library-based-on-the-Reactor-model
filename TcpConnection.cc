...
/**
 * 一般来说是在当前线程中执行该方法，因为TcpConnection对应的Channel最终注册到了某一线程的poller里面，poller去通知肯定也是在相应的loop线程里面去通知
 * 有一些应用场景是把connection记录下来，记录下来有可能是在其他线程调用connection进行send的数据发送
*/
...
/**
 * 发送数据 应用写得快(因为是非阻塞IO) 而内核发送数据慢 需要把待发送数据写入缓冲区，而且设置了水位回调
 **/
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        else // nwrote < 0
        {
            ...
            if (errno != EWOULDBLOCK) // EWOULDBLOCK表示非阻塞情况下没有数据后的正常返回 等同于EAGAIN
            {
                ...
                if (errno == EPIPE || errno == ECONNRESET) // 已经被对端关闭的 socket 调用 write()/send() 时，内核会：
//给你返回 -1，errno = EPIPE。同时向当前进程发送一个 SIGPIPE 信号。SIGPIPE 的默认行为是 终止进程。    
//ECONNRESET  场景：对端 异常关闭（比如强制断开、崩溃、reset）。  表现：通常你在 read() 时遇到，返回 -1，errno = ECONNRESET。说明 TCP 连接被 RST 包强制重置了。含义：连接不可恢复，必须关闭 fd。    ；SIGPIPE RESET
                {
                    faultError = true;
                }
            }
     ...
     * 相应的sock->channel，调用channel对应注册的writeCallback_（绑定的handleWrite）回调方法，
    ...
    {
        // 目前发送缓冲区剩余的待发送的数据的长度
        size_t oldLen = outputBuffer_.readableBytes();   //没能立即发出去的数据存放在了 outputBuffer_ 的 readable 区域，等待下次 handleWrite() 从这里“读出”并真正写进 socket
...
void TcpConnection::connectDestroyed()
{
    if (state_ == kConnected)
    {
        ...
        channel_->disableAll(); // 把channel的所有感兴趣的事件从poller中删除掉，调用epoll_ctrl
        connectionCallback_(shared_from_this());
    }
...
//当底层 socket 写缓冲区变为可写时，把应用层待发送的数据写出去，并在数据全部发送完成时触发回调 / 做连接关闭的后续处理
//有EPOLLOUT事件的时候执行
...
//poller =》channel：：closeCallback(Tcponnection::handleClose)
